...
export default class CreateData implements Seeder {
  public async run(_: any, connection: Connection): Promise<any> {
    ...
    const reactJsgroup = await Group.findOne({ where: { name: 'reactjs' } })
    const funnygroup = await Group.findOne({ where: { name: 'funny' } })
    const iqgroup = await Group.findOne({ where: { name: 'InsightfulQuestions' } })
    const oneliners = await Group.findOne({ where: { name: 'oneliners' } })
